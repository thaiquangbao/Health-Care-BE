...
class ExpiredLogBookService {
  async notification(logBooks) {
    const [hours, minutes] = logBooks.date.time
      .split(":")
      .map(Number);
    ...
    if (logBooks.priceList.price === 1350000) {
      const notificationDate = logBookDate
        .clone()
        .add(3, "months"); // cộng thêm 3 tháng
      if (now.isSameOrAfter(notificationDate)) {
        const completedData =
          await healthLogBookService.completed(
            logBooks._id
          );
        const dataPayback = {
        ...
        console.log(
          "Không có lịch hẹn 3 tháng nào hết hạn"
        );
      }
    } else if (logBooks.priceList.price === 2300000) {
      const notificationDate6 = logBookDate
        .clone()
        .add(6, "months"); // cộng thêm 6 tháng
      if (now.isSameOrAfter(notificationDate6)) {
        ...
        console.log(
          "Không có lịch hẹn 6 tháng nào hết hạn"
        );
      }
    } else {
      const notificationDate12 = logBookDate
        .clone()
        .add(12, "months"); // cộng thêm 12 tháng
      if (now.isSameOrAfter(notificationDate12)) {
        ...
        console.log(
          "Không có lịch hẹn 12 tháng nào hết hạn"
        );
      }
    }
  }
